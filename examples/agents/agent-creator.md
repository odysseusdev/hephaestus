...
7. verify: only the two known template tokens (`output`, `skills`) appear; every listed skill has a matching directory; `id` matches the intended filename; no file already exists at the target path.
