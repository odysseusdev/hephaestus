...
4. derive the slug (lowercase, hyphen-separated) and filename (descriptive of the content, e.g. `conventions.md`). search `<content-dir>/skills/` for the slug. if the skill directory does not exist yet, it will be created as part of writing the file. if the skill directory already exists, that is fine on its own. proceed to add the new file as a sibling, unless a file already exists at the exact target path `<content-dir>/skills/<id>/<filename>.md`, in which case stop and report the path collision. separately, if the caller's request describes the same scope as the existing skill (rather than a distinct new filename covering a distinct concern), stop and report the scope duplication instead of proceeding.
...
7. verify: frontmatter has only `description`. no section has fewer than 3 or more than 8 rules. no file already exists at the exact target path, and this isn't a duplicate-scope skill parceled into the existing directory's slug.
...
- do not overwrite an existing skill file without explicit instruction. this includes writing into an already-existing skill directory when the exact target filename is already taken. stop and report the path collision instead of overwriting.
