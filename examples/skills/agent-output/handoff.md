...
- most roles need nothing beyond the base template.
- an agent may append its own role-specific section after `follow-ups` when it adds real value beyond the base template.
- this skill never enumerates which agents extend the template or what their sections are called. that is specified inline in each agent's own steps, not registered here.
- an agent that already writes its own dedicated output document elsewhere in this skill (a decision, research, plan, or review document) must not add a handoff extension section that duplicates that document; its handoff stays pointer-only.
